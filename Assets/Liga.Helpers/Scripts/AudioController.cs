...
public class AudioController : MonoBehaviour
{
    ...
    public int SoundEffectChannel = 20;
    ...
    public void Play(AudioClip audioClip, SoundType type, float volume = 1f, bool loop = false, bool exclusiveSound = false, float duration = 0, float pitch = 1f)
    {
        ...
        switch (type)
        {
            ...
            case SoundType.SoundEffect2D:
                ...
                for (int i = 0; i < _soundEffectChannels.Count; i++)
                {
    ...
    {
        AudioSource currentSoud = _soundEffectChannels.FirstOrDefault(x => x.clip == sound && x.isPlaying);
        if (currentSoud != null)
        {
            //_soundEffectChannels.Remove(currentSoud);
            currentSoud.Stop();
        }
    }

    public void StopExclusive(AudioClip sound)
    {
        AudioSource currentSound = _exclusiveSounds.FirstOrDefault(x => x.audioSource.clip == sound).audioSource;
    ...
    {
        foreach(AudioSource a in _soundEffectChannels)
        {
            if(a.isPlaying)
            {
                a.Stop();
